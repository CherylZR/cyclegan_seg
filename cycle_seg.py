...
class CycleGANMaskModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionIdt = networks.CrossEntropyLoss2d()
    ...
    def ToDiscrete(self, continuous):
        ...
        mask = np.ceil(tensor * (tensor>self.thres))
        return torch.from_numpy(mask).cuda().type(torch.cuda.FloatTensor)

    def forward(self):
        self.label_MA = Variable(torch.squeeze(self.input_MA, 1))
        self.label_MB = Variable(torch.squeeze(self.input_MB, 1))
    ...
    def test(self):
        ...
        fake_maskB = torch.unsqueeze(fake_MB.data[:, 1], 1)
        rec_MA = self.netG_B(Variable(torch.cat([self.input_A, fake_maskB], 1)))
        self.rec_MA = torch.unsqueeze(rec_MA.data[:, 1], 1)
        self.fake_MB = fake_maskB
        fake_MA = self.netG_B(self.real_B)
        fake_maskA = torch.unsqueeze(fake_MA.data[:, 1], 1)
        rec_MB = self.netG_A(Variable(torch.cat([self.input_B, fake_maskA], 1)))
        self.rec_MB = torch.unsqueeze(rec_MB.data[:, 1], 1)
        self.fake_MA = fake_maskA
    ...
    def backward_G(self):
        ...
        # Cycle loss
        fake_MB = self.netG_A(self.real_A)
        fake_MB_1 = torch.unsqueeze(fake_MB.data[:, 1], 1)
        fake_AMB = Variable(torch.cat([self.input_A, fake_MB_1], 1))
        fake_AMB_dis = Variable(torch.cat([self.input_A, self.ToDiscrete(fake_MB_1)], 1))
        rec_MA = self.netG_B(fake_AMB_dis)
        loss_cycle_A = self.criterionCycle(rec_MA, self.label_MA.type(torch.cuda.LongTensor)) * lambda_A

        fake_MA = self.netG_B(self.real_B)
        fake_MA_1 = torch.unsqueeze(fake_MA.data[:, 1], 1)
        fake_BMA = Variable(torch.cat([self.input_B, fake_MA_1], 1))
        fake_BMA_dis = Variable(torch.cat([self.input_B, self.ToDiscrete(fake_MA_1)], 1))
        rec_MB = self.netG_A(fake_BMA_dis)
        loss_cycle_B = self.criterionCycle(rec_MB, self.label_MB.type(torch.cuda.LongTensor)) * lambda_B
        ...
        if lambda_idt > 0:
            ...
            idt_MA_1 = torch.unsqueeze(idt_MA.data[:, 1], 1)
            loss_idt_A = self.criterionIdt(idt_MA, self.label_MA.type(torch.cuda.LongTensor)) * lambda_A * lambda_idt
            # loss_idt_B:
            idt_MB = self.netG_A(self.real_B)
            idt_MB_1 = torch.unsqueeze(idt_MB.data[:, 1], 1)
            loss_idt_B = self.criterionIdt(idt_MB, self.label_MB.type(torch.cuda.LongTensor)) * lambda_B * lambda_idt

            self.idt_MA = idt_MA_1
            self.idt_MB = idt_MB_1
        ...
        self.fake_MA = fake_MA_1
        self.fake_MB = fake_MB_1
        self.rec_MA = torch.unsqueeze(rec_MA.data[:, 1], 1)
        self.rec_MB = torch.unsqueeze(rec_MB.data[:, 1], 1)
