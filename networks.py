...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if which_model_netG == 'resnet_9blocks':
        ...
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6,
                               gpu_ids=gpu_ids)
    elif which_model_netG == 'unet256':
        netG = Unet256(input_nc, ndf=64, norm_layer=nn.BatchNorm2d)
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    if which_model_netD == 'basic':
        ...
        netD = NLayerDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, use_sigmoid=use_sigmoid,
                                   gpu_ids=gpu_ids)
    elif which_model_netD == 'IncepD':
        netD = InceptionDiscriminator(input_nc, use_sigmoid=use_sigmoid, gpu_ids=gpu_ids)
    elif which_model_netD == 'n_layers_to1':
        netD = NLayerDiscriminator_to1(input_nc, ndf, n_layers=3, norm_layer=norm_layer, use_sigmoid=use_sigmoid,
                                       gpu_ids=gpu_ids)
    else:
...
class Unet256(nn.Module):
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d):
        ...
        self.ds1 = nn.Sequential(*[
            nn.Conv2d(4, 64, 4, 2, 1, bias=use_bias)
        ])
        ...
        self.ds3 = nn.Sequential(*[
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(128, 256, 4, 2, 1, bias=use_bias),
            nn.BatchNorm2d(256)
        ])
        self.ds4 = nn.Sequential(*[
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(256, 512, 4, 2, 1, bias=use_bias),
            nn.BatchNorm2d(512)
        ])
        self.ds5 = nn.Sequential(*[
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(512, 512, 4, 2, 1, bias=use_bias),
            nn.BatchNorm2d(512)
        ])
        ...
        self.us8 = nn.Sequential(*[
            nn.ReLU(True),
            nn.ConvTranspose2d(512, 512, 4, 2, 1, bias=use_bias),
            nn.BatchNorm2d(512)
        ])
        self.us7 = nn.Sequential(*[
            nn.ReLU(True),
            nn.ConvTranspose2d(1024, 512, 4, 2, 1, bias=use_bias),
            nn.BatchNorm2d(512)
        ])
        ...
        self.us4 = nn.Sequential(*[
            nn.ReLU(True),
            nn.ConvTranspose2d(1024, 256, 4, 2, 1, bias=use_bias),
            nn.BatchNorm2d(256)
        ])
        self.us3 = nn.Sequential(*[
            nn.ReLU(True),
            nn.ConvTranspose2d(512, 128, 4, 2, 1, bias=use_bias),
            nn.BatchNorm2d(128)
        ])
        self.us2 = nn.Sequential(*[
            nn.ReLU(True),
            nn.ConvTranspose2d(256, 64, 4, 2, 1, bias=use_bias),
            nn.BatchNorm2d(64)
        ])
        self.us1 = nn.Sequential(*[
            nn.ReLU(True),
            nn.ConvTranspose2d(128, 1, 4, 2, 1, bias=use_bias),
            nn.Tanh()
        ])
        # self.cls1 = nn.Conv2d(1, 1, 256, bias=use_bias)
        # self.cls2 = nn.Conv2d(1, 1, 256, bias=use_bias)
    ...
    def forward(self,x):
        ...
        ds7 = self.ds7(ds6)
        ds8 = self.ds8(ds7)
        # print(ds1.size(),ds2.size(),ds3.size(),ds4.size(),ds5.size(),ds6.size(),ds7.size(),ds8.size())
        us8 = torch.cat([ds7, self.us8(ds8)], 1)
        us7 = torch.cat([ds6, self.us7(us8)], 1)
        us6 = torch.cat([ds5, self.us6(us7)], 1)
        us5 = torch.cat([ds4, self.us5(us6)], 1)
        us4 = torch.cat([ds3, self.us4(us5)], 1)
        us3 = torch.cat([ds2, self.us3(us4)], 1)
        us2 = torch.cat([ds1, self.us2(us3)], 1)
        # print(us8.size(),us7.size(),us6.size(),us5.size(),us4.size(),us3.size(),us2.size())
        us1 = self.us1(us2)
        # print(us1)
        return us1
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[]):
        ...
        self.weight_1 = Parameter(torch.normal(std=0.02 * torch.ones(256, 256, 4, 4)))
        self.model_2 = nn.Sequential(*sequence_2)
        #self.conv = nn.Conv2d()

    def forward(self, input, flag):
        if len(self.gpu_ids) and isinstance(input.data, torch.cuda.FloatTensor):
            out = nn.parallel.data_parallel(self.model_1, input, self.gpu_ids)
            if flag == 1:
                out = func.conv2d(out, self.weight_1, stride=2, padding=1)
            out = nn.parallel.data_parallel(self.model_2, out, self.gpu_ids)
        else:
            out = self.model_1(input)
            if flag == 1:
                out = func.conv2d(out, self.weight_1, stride=2, padding=1)
            out = self.model_2(out)
            print('D forward out is:', out)
        return out'''
...
class NLayerDiscriminator_to1(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[]):
        super(NLayerDiscriminator_to1, self).__init__()
        self.gpu_ids = gpu_ids
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
        ...
        sequence += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]
        sequence += [nn.Conv2d(1, 1, kernel_size=30, stride=1, padding=0)]
...
class InceptionDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, use_sigmoid=False, gpu_ids=[]):
        ...
        #     nn.LeakyReLU(0.2, True)
        # ]
        # sequence += InceptionA(256)
        # sequence += InceptionB(384)
        # sequence += InceptionA(192)
        # sequence += nn.Conv2d(192, 1, kernel_size = 3, padding=1)
        #
        # self.model = nn.Sequential(*sequence)
        ...
        self.inception3 = InceptionB(384)
        self.last2 = nn.Conv2d(384, 1, kernel_size=3, padding=1)
